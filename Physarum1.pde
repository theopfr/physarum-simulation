...
int cellAmount = 50000;  // affects frame rate
...
float size = 1;
float speed = 1;
float pheromoneDecay = 0.975;  // 0.925 - 0.99 affects frame rate
...
void initializeCells() {
  for (int i=0; i<cellAmount; i++) {
    ...
    if (initializeToCenter) {
      ...
      //float angle = random(TWO_PI);
      //initialDirectionVector = PVector.fromAngle(angle).normalize();
    }
    else {
...
int sensePheromones(PVector cellPosition, PVector cellDirection) {
  ...
  //PVector ext0 = turnVector(new PVector(cellDirection.x, cellDirection.y), (int) random(-1, 1) * randomSteerStrength).normalize();
